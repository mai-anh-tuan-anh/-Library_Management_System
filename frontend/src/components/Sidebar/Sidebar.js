...
const menuItems = [
  ...
  { path: '/borrowing', label: 'Mượn sách', icon: RiExchangeLine },
  { path: '/returns', label: 'Trả sách', icon: RiArrowGoBackLine },
  { path: '/due-alerts', label: 'Cảnh báo hạn trả', icon: RiAlarmWarningLine },
    ...
    <>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
          <div className={`flex items-center gap-3 mb-3 ${!isOpen && 'lg:hidden'}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">{user?.email || 'admin@library.vn'}</p>
            </div>
          </div>
          ...
          >
            ...
            <span className={`${!isOpen && 'lg:hidden'}`}>Đăng xuất</span>
          </button>
        </div>
      </aside>
    </>
