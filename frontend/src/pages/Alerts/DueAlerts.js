...
import { RiAlarmWarningLine, RiPhoneLine, RiMailLine, RiCheckLine, RiNotificationLine } from 'react-icons/ri';
...
const DueAlerts = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Cảnh báo hạn trả</h1>
        <p className="text-gray-500 mt-1">Theo dõi và gửi nhắc nhở cho độc giả</p>
      </div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Sắp đến hạn (≤3 ngày)</p>
            </div>
          </div>
        </div>
        <div className="card p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold">
                {formatCurrency(overdue.reduce((acc, item) => acc + (item.estimated_fine || 0), 0))}
              </p>
              <p className="text-sm text-gray-500">Phạt ước tính</p>
            </div>
          </div>
        </div>
      </div>
              ...
              {dueSoon.map((alert, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="table-cell">
                    <div>
                      <p className="font-medium">{alert.reader_name}</p>
                      <p className="text-sm text-gray-500">{alert.card_number}</p>
                    </div>
                  </td>
                  <td className="table-cell">
                    <p className="font-medium">{alert.book_title}</p>
                    <p className="text-sm text-gray-500 font-mono">{alert.barcode}</p>
                  </td>
                  <td className="table-cell">
                    {format(new Date(alert.expected_return_date), 'dd/MM/yyyy', { locale: vi })}
                  </td>
                  <td className="table-cell">
                    <span className={`badge ${
                      alert.days_remaining <= 1 ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{alert.reader_phone}</span>
                    </div>
                  </td>
                  <td className="table-cell">
                    <button
                      onClick={() => sendReminder(alert.transaction_id, alert.days_remaining)}
                      className="btn-primary py-1 px-3 text-sm gap-1"
                    >
                    ...
                    <div className="space-y-1">
                      ...
                      {alert.reader_email && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">{alert.reader_email}</span>
                        </div>
                      )}
                    </div>
                  </td>
