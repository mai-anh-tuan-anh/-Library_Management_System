  ...
  const loadReports = async () => {
    ...
    try {
      if (activeTab === 'revenue') {
        ...
        // Use monthly data for the chart
        if (monthlyRes.data) {
          const labels = monthlyRes.data.slice(0, 6).map(r => r.month_name).reverse();
          const borrowData = monthlyRes.data.slice(0, 6).map(r => r.borrow_revenue).reverse();
          const fineData = monthlyRes.data.slice(0, 6).map(r => r.fine_revenue).reverse();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Xem báo cáo doanh thu, sách mượn nhiều, độc giả tích cực</p>
        </div>
      ...
      <div className="border-b border-gray-200">
        <nav className="flex gap-8">
          {[
            { id: 'revenue', label: 'Doanh thu', icon: RiBarChartLine },
            { id: 'top-books', label: 'Top sách', icon: RiBookLine },
            { id: 'top-readers', label: 'Top độc giả', icon: RiUserLine },
          ].map((tab) => (
            <button
              key={tab.id}
      ...
      {activeTab === 'top-books' && (
        <div className="card overflow-hidden">
          <div className="p-4 bg-primary-50 border-b">
            <h3 className="font-semibold text-primary-800">Top 10 sách được mượn nhiều nhất</h3>
          </div>
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{book.book_code}</p>
                      </div>
                    </div>
                  </td>
                  <td className="table-cell">{book.category_name || '-'}</td>
                  <td className="table-cell">
                    <span className="badge bg-blue-100 text-blue-800">
                      {book.total_borrows} lượt
                    </span>
                  </td>
                  <td className="table-cell">{book.unique_readers} người</td>
                  <td className="table-cell font-medium">{formatCurrency(book.total_revenue)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {activeTab === 'top-readers' && (
        <div className="card overflow-hidden">
          <div className="p-4 bg-green-50 border-b">
            <h3 className="font-semibold text-green-800">Top 10 độc giả tích cực</h3>
          </div>
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <tr key={reader.reader_id} className="hover:bg-gray-50">
                  ...
                  <td className="table-cell">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{reader.card_number}</p>
                    </div>
                  </td>
                  <td className="table-cell">
                  ...
                  <td className="table-cell">{reader.unique_books_borrowed} cuốn</td>
                  <td className="table-cell font-medium">{formatCurrency(reader.total_borrow_fees)}</td>
                </tr>
              ))}
            </tbody>
          </table>
