...
const BorrowTransaction = () => {
    ...
    const addBook = (book) => {
        ...
        if (book.status !== 'available') {
            toast.error('Bản sao sách này đang được mượn, không thể thêm');
            return;
        }

        // Check membership tier limit
        const maxBooks = selectedReader?.max_books || 5;
        if (selectedBooks.length >= maxBooks) {
            toast.error(
                `Độc giả chỉ được mượn tối đa ${maxBooks} cuốn sách (theo hạng thành viên)`
            );
            return;
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Check response structure - service already unwraps response.data
            const transactionId = txResponse.data?.transaction_id;

            if (!transactionId) {
                console.error('Response structure:', txResponse);
                throw new Error('Không nhận được mã phiếu mượn từ server');
            }

            // Add books
            for (const book of selectedBooks) {
                await borrowService.addBook(transactionId, {
                    copy_id: book.copy_id,
                    borrow_days: book.borrow_days,
                    daily_fee: book.daily_fee
                });
                // Small delay to prevent MySQL deadlock
                await new Promise((resolve) => setTimeout(resolve, 100));
            }

            // Finalize with retry on deadlock
            let finalizeSuccess = false;
            let finalizeRetries = 0;
            while (!finalizeSuccess && finalizeRetries < 3) {
                try {
                    await borrowService.finalize(transactionId);
                    finalizeSuccess = true;
                } catch (err) {
                    finalizeRetries++;
                    if (finalizeRetries >= 3) throw err;
            ...
            {step === 2 && (
                <div className='space-y-4'>
                    ...
                    <div className='flex justify-between'>
                        ...
                        <button
                            onClick={() =>
                                selectedBooks.length > 0 && setStep(3)
                            }
